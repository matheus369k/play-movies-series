...
export function MoviesInFocus() {
    ...
    return (
        <section className="bg-focus-movie">
            {movies &&
                movies.map((movie) => (
                    <div className={`relative max-w-7xl mx-auto w-full min-h-screen flex items-center flex-col gap-10 justify-end pb-10`}>
                        <div key={movie.id} className="flex items-center flex-col gap-10 max-w-7xl">
                            <div className="relative group/play bg-black/50 rounded-md border border-gray-100 w-max h-max z-50">
                                ...
                                <button
                                    className="invisible absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border border-gray-100 bg-gray-200/20 rounded-full p-5 cursor-pointer transition hover:bg-gray-200/10 group-hover/play:visible"
                                    type="button">
                        ...
                        <button
                            className="border border-gray-100 bg-gray-200/20 rounded-full p-5 hover:bg-gray-200/10 cursor-pointer transition-all"
                            type="button">
                            <FaPlay className="size-10" />
                        </button>
