...
describe('loginUser', () => {
  ...
  it('should request return corrected', async () => {
    MockAxiosBackApi.onPost('/users/login', userRequest).replyOnce(200, {
      user: userResponse,
      token: jwtToken,
    })
    const { result } = renderHook(() => loginUser(userRequest))
  ...
  it('should handle error when request for recused', async () => {
    ...
    MockAxiosBackApi.onPost('/users/login', userRequest).replyOnce(
      500,
      undefined
    )
    const { result } = renderHook(() => loginUser(userRequest))
  ...
  it('should handle empty response when fetching login user data', async () => {
    const consoleSpy = jest.spyOn(console, 'log')
    MockAxiosBackApi.onPost('/users/login', userRequest).replyOnce(
      200,
      undefined
